-- Call Tracking and Usage Management Schema
-- This migration adds tables for call tracking, CDR uploads, usage tracking, and voice integrations

-- Call attempts tracking
CREATE TABLE IF NOT EXISTS call_attempts (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  organization_id UUID NOT NULL REFERENCES organizations(id) ON DELETE CASCADE,
  call_list_contact_id UUID REFERENCES call_list_contacts(id) ON DELETE SET NULL,
  ...
  disposition VARCHAR(50), -- answered, voicemail, busy, no_answer, failed, callback, dnc, sale, appointment
  disposition_notes TEXT,
  recording_url TEXT,
  recording_sid VARCHAR(255), -- SignalWire recording ID
  transcription TEXT,
  ai_summary TEXT,
...
COMMENT ON TABLE call_attempts IS 'Tracks all call attempts made through the platform, including manual CDR uploads';
